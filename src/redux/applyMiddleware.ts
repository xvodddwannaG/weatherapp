...
import { setCardData, errorHandler, setIsCityNameValid } from './actionCreator';
...
export const getWeather = (inputCity: string) => async (dispatch: any) => {
  try {
    ...
    dispatch(setCardData(name, roundTemp));
    dispatch(setIsCityNameValid(true));
    localStorage.setItem('history', JSON.stringify({ city: name, temp: roundTemp }));
  } catch (error) {
    dispatch(errorHandler(error));
    dispatch(setIsCityNameValid(false));
  }
};
